...
import { availableCompletedRounds, buildHarePickMap, buildTortoisePickMap, currentLeaderboardRound, entryMovementSincePriorRank, leaderboardForCompletedRound, leaderboardForRoundOnly, normalizePickName, rankEntries, scoreEntry, type EntryMovement, type PickScore, type ScoredEntry } from '@/lib/scoring'
...
function InlineLeaderboard({ pool, entries, currentEntryId, openEntryIds, onEntryToggle, teeTimeZone, mode }: {
  ...
  const currentRoundNumber = leaderboardModeIsCurrent ? currentLeaderboardRound(baseLeaderboardRows) : null
  ...
  const totalScoreSubLabel = leaderboardMode.type === 'current'
    ? (currentRoundLabel && (availableHistoricalRounds.length > 0 || isAfterOpeningRoundDate(tournament)) ? currentRoundLabel : null)
