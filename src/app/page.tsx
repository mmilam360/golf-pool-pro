...
export default function Home() {
  ...
  return (
    <div className="min-h-screen scorecard-paper text-stone-900">
      <main className="max-w-6xl mx-auto px-6 py-16 md:py-20">
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-10 items-center mb-16">
          ...
          <section className="bg-white/95 rounded-xl border-4 championship-border shadow-sm overflow-hidden">
            <div className="flex items-center justify-between border-b border-stone-200 bg-emerald-900 px-5 py-4 text-white">
              <div>
                <p className="text-xs uppercase tracking-[0.16em] text-emerald-100">Sunday Board</p>
                <h2 className="font-display text-2xl font-semibold">Pool Leaderboard</h2>
              </div>
            ...
            <div className="grid grid-cols-3 gap-px bg-stone-200 text-center text-sm">
              {['12 picks', '8 scores', '$40 buy-in'].map(stat => (
                <div key={stat} className="bg-amber-50 px-3 py-4 font-script text-lg text-stone-800">{stat}</div>
              ))}
            </div>
          </section>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {[
            ...
            <div key={f.title} className="bg-white/95 rounded-xl p-6 border border-stone-200 shadow-sm">
              <h3 className="font-display text-xl font-semibold mb-2 text-emerald-950">{f.title}</h3>
