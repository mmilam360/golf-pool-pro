...
export async function POST(request: Request) {
  try {
    const { poolId, force = false } = await request.json()
    ...
    // Phase 1: Try PGA Tour fresh field
    ...
    // Phase 2: Fallback to stored field
    ...
    // Phase 3: Quality gate (allow force override)
    ...
    if (!quality.ok) {
      return NextResponse.json({
        ok: false,
        error: `Field failed quality check: ${quality.reason}`,
        canForce: true,
