...
import { getPoolPaymentQuote, getPoolPaymentStatus, getRedeemedPromoCreditCents } from '@/lib/payments/pricing'
...
async function getOwnedPoolQuote(poolId: string) {
  ...
  try {
    ...
    const { data } = await serviceSupabase
      ...
      .maybeSingle()
    redeemedPromo = data
    const promo = Array.isArray(data?.gpp_promo_codes) ? data.gpp_promo_codes[0] : data?.gpp_promo_codes
    promoCreditCents = data ? getRedeemedPromoCreditCents(count || 0, promo, Number(data.discount_cents || 0)) : 0
  } catch {}

  const effectiveAmountPaidCents = amountPaidCents + promoCreditCents
  const quoteForStatus = getPoolPaymentQuote(count || 0, effectiveAmountPaidCents)
  const quote = { ...quoteForStatus, amountPaidCents, promoCreditCents }
  ...
  const paymentStatus = getPoolPaymentStatus(storedPaymentStatus, count || 0, effectiveAmountPaidCents)
  let claimedPromo: any = null

  try {
    if (serviceSupabase && !redeemedPromo && quote.amountDueCents > 0) {
