...
import { getPoolPaymentQuote } from '@/lib/payments/pricing'
...
export async function POST(request: Request) {
  try {
    ...
    const { poolId, sourceId } = await request.json()
    ...
    if (quote.amountDueCents <= 0) {
      await supabase
        ...
        .eq('id', poolId)
      return NextResponse.json({ success: true, amountDueCents: 0, entryLimit: quote.entryLimit })
    }
    ...
    const idempotencyKey = `gpp_${compactPoolId}_${activeEntryCount}_${quote.amountDueCents}`
    ...
    const nextAmountPaid = amountPaidCents + quote.amountDueCents
    ...
    const { error: insertPaymentError } = await serviceSupabase.from('gpp_pool_payments').upsert({
      ...
      amount_cents: quote.amountDueCents,
