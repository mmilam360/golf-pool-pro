...
export async function POST(request: Request) {
  try {
    ...
    if (promoCode && quote.amountDueCents > 0) {
      ...
      if (firstOwnedPool?.id !== poolId) {
        return NextResponse.json({ error: 'This offer only applies to your first pool.' }, { status: 409 })
      }
    ...
    if (savedCardId) {
      const { data: savedCard, error: savedCardError } = await serviceSupabase
        ...
        .select('id, square_customer_id, square_card_id, brand, last_4')
        .eq('id', savedCardId)
        .eq('user_id', user.id)
        .maybeSingle()
    ...
    return NextResponse.json({
      ...
      savedCard: savedCardForResponse,
    })
  } catch (error: any) {
