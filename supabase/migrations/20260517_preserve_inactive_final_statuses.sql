...
begin
  ...
  select jsonb_agg(
    case
      ...
      when lower(coalesce((old_by_name -> (player->>'name'))->>'status', '')) in ('cut', 'wd', 'dnq')
        and lower(coalesce(player->>'status', '')) = 'active'
        then player
          || jsonb_build_object(
            'status', lower((old_by_name -> (player->>'name'))->>'status'),
            'thru', '',
            'roundScore', '',
            'position', case lower((old_by_name -> (player->>'name'))->>'status')
              when 'wd' then 'WD'
              when 'dnq' then 'DNQ'
              else 'CUT'
            end,
            'score', case lower((old_by_name -> (player->>'name'))->>'status')
              when 'wd' then 'WD'
              when 'dnq' then 'DNQ'
              else coalesce(player->>'score', '')
            end
          )
      else player
        || case
