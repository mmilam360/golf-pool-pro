  ...
  select jsonb_agg(
    case
      ...
      when not (old_by_name ? (player->>'name')) then player
        ...
        || case when not (player ? 'teeTime') and (old_by_name -> (player->>'name')) ? 'teeTime'
          then jsonb_build_object('teeTime', (old_by_name -> (player->>'name'))->'teeTime')
          else '{}'::jsonb end
        || case when not (player ? 'startTee') and (old_by_name -> (player->>'name')) ? 'startTee'
          then jsonb_build_object('startTee', (old_by_name -> (player->>'name'))->'startTee')
          else '{}'::jsonb end
        || case when not (player ? 'roundScore') and (old_by_name -> (player->>'name')) ? 'roundScore'
          then jsonb_build_object('roundScore', (old_by_name -> (player->>'name'))->'roundScore')
          else '{}'::jsonb end
        || case when not (player ? 'roundScores') and (old_by_name -> (player->>'name')) ? 'roundScores'
          then jsonb_build_object('roundScores', (old_by_name -> (player->>'name'))->'roundScores')
          else '{}'::jsonb end
    end
  )
  into merged
