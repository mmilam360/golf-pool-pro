...
- 3D depth correction: use old-school extruded-box construction with a front face plus skewed parallelogram right/bottom depth faces so the corners read diagonal. Avoid fragile `clip-path` notches/wedges and negative-z oddities on the main board faces because they create disconnected artifacts on mobile. On live pool/dashboard/homepage board posts, do not use a transformed side face that extends above/below the post — it creates a mismatched top green sliver and a hanging bottom shard. Use a clipped side face constrained inside the post height and a solid green post face. On story/final-result exports, the screenshot export keeps the accepted mounted-through-board look with the post continuing off the bottom of the image.
