...
- 3D depth correction: use old-school extruded-box construction with a front face plus skewed parallelogram right/bottom depth faces so the corners read diagonal. Avoid fragile `clip-path` notches/wedges and negative-z oddities that create disconnected artifacts on mobile. On live boards and story/final-result exports, the post front should start halfway down the board's dark bottom extrusion so half of that bottom face remains visible above the post, while the post's dark side extrusion extends upward into the board bottom depth. This makes the post read as mounted through the center of the board thickness. On story exports the post should continue off the bottom of the image; on homepage hero it should run into the next green section with no blank gap below it.
